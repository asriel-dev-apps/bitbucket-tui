...
- **v0.1.17 (2026-07-19)**: M10。認証情報保存を刷新。macOS は再ビルド時の Keychain ACL 変更を避けるため `/usr/bin/security` を仲介し、Linux は libsecret を実行時 dlopen して Secret Service を利用する。headless 向けに非永続の `BBTUI_EMAIL` / `BBTUI_TOKEN` 起動経路を追加し、keyring crate を削除。詳細は `specs/M10.md` / `LEDGER.md`。
- **v0.1.16 (2026-07-19)**: M9。実運用フィードバック第 4 陣。(1) フッターのキーヒント並びを反転（`Enter` を先頭・`?` を末尾へ。狭い幅でも `?` は幅予約で必ず表示）。(2) ソートラベルを英語化（Recently Updated / Least Recently Updated / Newest / Oldest）。(3) 同一スレッド内のコメント間に枠内空行（`InnerSpacer`）。(4) フィルタモーダルの Author / Target branch を States と同じ**チェックボックス複数選択 + 作業コピー**へ統一（Space=即時トグル・Enter=適用・Esc=破棄。All は常時 row 0、選択済みは All 直下に pin。BBQL は `(x OR y)` 連結で複数指定。OR は公開リポジトリで実測済み）。詳細は `specs/M9.md` / `LEDGER.md`。
- **v0.1.15 (2026-07-18)**: M8。一覧・Diff の磨き込み第 2 陣。(1) フッターのキーヒント方針を刷新（`?` を先頭固定・PgUp/PgDn と Shift+J/K を全画面から削除・PR 一覧はフィルタ系を `f` のみに）。(2) コメントの吹き出しを 💬 に戻す。(3) unified 差分に旧/新 2 列の行番号ガター。(4) フィルタモーダルに author の文字列検索（そのまま入力で fuzzy 絞り込み）と **Target branch フィルタ**（自由入力=部分一致 `~`・候補選択=完全一致 `=`）を追加。(5) author 候補をワークスペースメンバーから「この repo の PR 著者集約（最新 150 件）」へ切替（members API 依存を削除。権限保有者一覧 API は管理者限定のため）。(6) `✔n/m` バッジはレビュアー 0 のとき非表示。詳細は `specs/M7.md` の M8 節 / `LEDGER.md`。
- **v0.1.14 (2026-07-17)**: M7。Phase 1=コメント UI の磨き込み（スレッド間に空行 / コメントエディタにカーソル表示と移動 Left/Right/Home/End/Up/Down・途中挿入削除 / 選択ハイライトを fg+bg 両上書きにして可読化=全テーマで selection_bg と accent が同色だったため / フッターのキーヒントを幅に収まるエントリのみ描画）。Phase 2=PR フィルタ（`f` でモーダル。state の複数選択 + author 指定。author 指定時は `q=state IN (...) AND author.uuid=...` に一本化=実測で `q` と `state` の併用不可のため。author 候補はワークスペースメンバー API+失敗時は読み込み済み PR 著者へフォールバック。states は config.toml 永続化）。詳細は `specs/M7.md` / `LEDGER.md`。
- **v0.1.13 (2026-07-16)**: Diff コメントの操作性を Bitbucket Web に寄せる第 3 弾。(1) コメントの時刻を 24 時間以内は相対表示（just now / Nm ago / Nh ago。PR 詳細ペインも同様）。(2) アイコンをモノクロ吹き出し（🗨）に変更。(3) 各コメントの下に `Reply · Resolve(ルートのみ) · Edit · Delete(自分のみ)` のアクションリンク行を表示し、クリックでも動作（`AppLayout.comment_actions` のヒットボックス）。`r` の返信先は選択中コメント（ネスト返信）。(4) スレッドの折りたたみ（Enter/クリック。解決済みは自動で折りたたみ、1 行のサマリ表示）。(5) 本文クリックで行選択/コメント選択。unified/split 両対応。詳細は `specs/M6.md` / `LEDGER.md`。
...
- モジュール構成:
  ...
  - `auth` — 認証情報: macOS Keychain（`/usr/bin/security`）/ Linux Secret Service（libsecret）へ API token を保存/読込/削除
