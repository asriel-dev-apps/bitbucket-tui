...
**進行中パイプラインの自動ポーリング更新**。さらに **リポジトリブラウズ**:
**ブランチ一覧 → コミット履歴 → コミット詳細 → コミット差分（Diff 流用）**、および
**ソースツリー閲覧（ディレクトリ潜行 / 親戻り）→ ファイル内容ページャ**（閲覧専用）。
...
| 画面 | キー | 動作 |
|------|------|------|
| リポジトリ | `b` | 選択リポジトリの **ブランチ一覧** を開く |
| リポジトリ | `s` | **ソース**（既定ブランチのルート）を開く |
| PR 一覧 | `b` / `s` | 同じリポジトリの **ブランチ一覧** / **ソース** へ切替 |
| ブランチ一覧 | `Enter` | そのブランチの **コミット履歴** を開く |
| ブランチ一覧 | `s` | そのブランチの **ソースルート** を開く |
| ブランチ一覧 | `r` | 一覧を再読込 |
| コミット履歴 | `Enter` | **コミット詳細** を開く |
| コミット履歴 | `r` | 履歴を再読込 |
| コミット詳細 | `d` | このコミットの **Diff**（色付き・M1 ビューア流用）を開く |
| コミット詳細 | `↑↓` / `j` `k` / `PgUp` `PgDn` | メッセージをスクロール |
| ソース | `Enter` | ディレクトリなら潜る / ファイルなら内容を表示 |
| ソース | `Backspace` / `Esc` | 親ディレクトリへ（ルートでリポジトリ一覧へ戻る） |
| ソース | `r` | 再読込 |
| ファイル | `↑↓` / `j` `k` | 1 行スクロール |
| ファイル | `PgUp` / `PgDn` | 1 画面スクロール |
| ファイル | `g` / `G` | 先頭 / 末尾へ |
| ファイル | `Esc` | ソースへ戻る |

ソースツリーはディレクトリ（`名前/`・シアン）→ ファイル（名前 + サイズ）の順に並ぶ。ヘッダに
`src ブランチ:/パス` を表示する。ファイルが **バイナリ**（mimetype または内容の NUL バイトで判定）
のときは「(バイナリ表示不可)」、**巨大ファイル**は先頭 5000 行 + 打切り注記を表示する。
コミット差分は PR の Diff と同じ操作・色分け（`+` 緑 / `-` 赤 / `@@` シアン / ファイル見出し 黄）。
...
### リポジトリブラウズ一巡（実 token が必要・任意）

テスト用リポジトリで、ブランチ／コミット／ソースを通しで確認する:

1. リポジトリ一覧で `b` を押して **ブランチ一覧** を開く。各行に
   `ブランチ名 / 最終コミット短縮hash / 日時 / 概要1行` が並ぶ。`r` で再読込。
2. ブランチを `Enter` で開き **コミット履歴**（`短縮hash / 日時 / author / メッセージ1行目`）を確認。
3. コミットを `Enter` で開き **コミット詳細**（hash / author / date / parents / メッセージ全文）を確認。
   `d` で **Diff** を開き、`↑↓`/`PgUp`/`PgDn`/`g`/`G` でスクロール、`n`/`N` でファイル境界移動、色分けを確認。
   `Esc` でコミット詳細へ戻る。
4. リポジトリ一覧（または PR 一覧）で `s` を押して **ソース**（既定ブランチのルート）を開く。
   ヘッダの `src ブランチ:/パス` とディレクトリ→ファイルの並びを確認。
5. ディレクトリを `Enter` で潜り、`Backspace`/`Esc` で親へ戻る（ルートでリポジトリ一覧へ戻る）ことを確認。
6. ファイルを `Enter` で開き、内容を `↑↓`/`PgUp`/`PgDn`/`g`/`G` でスクロール。バイナリは「(バイナリ表示不可)」、
   巨大ファイルは先頭 5000 行 + 打切り注記になることを確認。`Esc` でソースへ戻る。
7. 404／エラー（存在しない revision・パス等）がステータス行に表示されることを確認。

> 実 API で判明した src の dir/file 応答形・diff spec・commit/branch/SrcEntry のフィールド・
> revision/パスの URL エンコード挙動は `docs/LEDGER.md` の「未検証の仮定」に反映すること。
