...
- `docs/specs/M*.md` — 各マイルストーンの実装仕様書
...
- **秘密情報(API token)は平文ファイルに置かない**。永続化は macOS Keychain（`/usr/bin/security`）/ Linux Secret Service（libsecret）のみ。`BBTUI_EMAIL` / `BBTUI_TOKEN` は非永続の起動経路
